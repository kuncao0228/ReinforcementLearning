...
from ActorModel import ContinuousActor
...
class PPOAgent(object):
	...
	def __init__(
		...
		self.actor = ContinuousActor(self.obs_dim, act_dim).to(self.device)
		...
		for step_ in range(self.total_rollouts):
			...
			if self.solved_reward is not None:
				if np.mean(self.scores[-10:]) > self.solved_reward:
					...
					break
			value = self.critic(torch.FloatTensor(next_state).to(self.device)).to(self.device)
			self.memory.values.append(value)
			# update policy
